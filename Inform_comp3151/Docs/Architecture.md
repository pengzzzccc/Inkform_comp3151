...
| 编号 | 决策 | 备选 | 倾向（可改） |
|---|---|---|---|
| ① | 引擎解耦路线 | A 全量 / B 规则引擎式 / C 纯逻辑先下沉 | C→A 渐进（M0/M1 纯逻辑下沉，M2 才引入物理端口） |
| ② | 首个执行会话范围 | P0+M0 / P0+M0+M1 | P0+M0 先行（半天到两天，交付 Core 骨架） |
| ③ | 录制工具形态 | 离线回放 / 双模对比优先 | 双模对比为主、离线回放兜底 |
| ④ | 旧 Bus 文件去留 | M1 迁移后直接删除 / 保留一个版本作回退 | 直接删除（git 历史可回退） |
| ⑤ | 分支策略 | 主分支渐进 + 阶段标签 / 每阶段一个分支 | 主分支渐进 + 标签 |
